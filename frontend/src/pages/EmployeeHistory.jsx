  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const res = await API.get(`/employee/transactions${params}`);
    ...
    <div style={{ minHeight: '100vh', background: 'var(--color-bg)', padding: '2rem', fontFamily: 'var(--font-body)' }}>
      <div style={{ maxWidth: '700px', margin: '0 auto' }}>
        ...
        <p style={{ color: 'var(--color-text-secondary)', marginBottom: '1.25rem' }}>
          Tap any purchase for full details.
        </p>
        ...
        <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: '10px', padding: '1.25rem', marginBottom: '1.25rem' }}>
          <div style={{ fontSize: '0.7rem', color: 'var(--color-text-muted)', textTransform: 'uppercase' }}>Total Spent (Approved)</div>
          <div style={{ fontSize: '1.6rem', fontWeight: '800', color: 'var(--color-text)' }}>£{approvedTotal.toFixed(2)}</div>
        </div>
                ...
                <div key={t.id} style={{ borderBottom: i < transactions.length - 1 ? '1px solid var(--color-border)' : 'none' }}>
                  ...
                  {expanded && (
                    <div style={{ padding: '0 1.25rem 1rem', fontSize: '0.85rem', color: 'var(--color-text-secondary)', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem 1rem' }}>
                      <div><span style={{ color: 'var(--color-text-muted)' }}>Merchant:</span> {t.business_name}</div>
                      ...
                      <div><span style={{ color: 'var(--color-text-muted)' }}>Date/Time:</span> {new Date(t.created_at).toLocaleString()}</div>
                      {t.approved_at && (
                        <div style={{ gridColumn: '1 / -1' }}>
                          <span style={{ color: 'var(--color-text-muted)' }}>Approved:</span> {new Date(t.approved_at).toLocaleString()}
                        </div>
                      )}
                    </div>
                  )}
